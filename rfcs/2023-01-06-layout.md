...
This proposal outlines Klear360's approach towards _the much requested_ layout primitives and components. It covers the proposed components, APIs along with prior art and alternatives considered.

### Background

In the React ecosystem, there are a plethora of ways to do layouts. With this design, our aim is to strike a fine balance between flexibility and constraints, so everything fits nicely with Klear360's philosophy.

Since Klear360 is a [cross platform design system](https://portal.gitnation.org/contents/the-sorcery-of-building-a-cross-platform-design-system-architecture) that works in both React and React Native _(Learn Once, Write Anywhere)_; we've to make the layout strategy work within the constraints of both.
...
```tsx
import { Box, Button, Heading, Text } from '@klear/klear360/components';
...
At the moment, we don't provide any primitives from Klear360 to help with composing multiple UI elements to build a larger layout. Consider a small example with two buttons aligned vertically and some spacing in between:
...
1. Need for a lot of wrapper components _(wrapocalypse)_ for spacing and alignment, which quickly grows out of hand as UI evolves. One can't add styling for layout directly on a Klear360 component because we don't support `className` or `style` prop, which makes wrappers a necessity even for small things.
...
1. Considering Klear360 is aimed at bridging cross platform gaps, you still need to write JSX with platform specific elements such as `div`, `View`.

#### `klear360-old`

Historically, in `klear360-old` _(deprecated)_ we followed an enhancer components based pattern which looked like below:
...
Indubitably, layouts have been one of the most requested features by developers using Klear360 _([link](https://docs.google.com/document/d/14VAfBW7vz6hYOC1eWQO-_wOcECLU9UXRHVOSPC6OxZ8/edit#heading=h.zg0vxpo7xuc8) to internal document)_.
...
### Layout primitives and components for Klear360

In order to provide a good balance of flexibility and constraints within klear360 we should support the theme aware components and primitives listed below.
...
Since klear360 is not a general purpose design system we should lean more towards constraints initially, assess usecases in the future and then increase flexibility. Proposed props for initial version:
...
We have an [internal Box component](../packages/klear360/src/components/Box/) but it exposes lot more properties than the ones associated with layout. It seems like a good idea to have a different public `Box` component with the restrictive but fully covered layout properties from above table.
...
All presentational components in Klear360 will support a subset of layout specific properties as top level props. The values will also be same as the ones supported in `Box`. Sample usage:
...
Styling props should be the preferred choice for minor adjustments only. You may note `padding*` is skipped since it can break existing padding of klear360 components _(users may override padding of `Button`, `Alert`, etc.)_. Supported restricted properties:
...
- `className` and `style` props aren't supported. A drawback of this is people relying on maintaining their styles separately (`.css` files) wouldn't be able to set custom CSS properties on klear360 components. They'll have to rely on the supported primitives and components. As a constraint this helps us to keep the components less prone to getting styled unexpectedly _(a `padding` property on CSS can mess up the built-in spacing on a component)_. For example - exposing `className` enables users to [style the component](https://styled-components.com/docs/basics#styling-any-component) to anything.
...
1. Storybook guideline doc for understanding how to work with layouts in Klear360, talking about best practices and common examples.
...
```tsx
import { useSx } from '@klear/klear360/utils';
...
#### Whether styled-props get more priority over defined props in klear360 or the other way
