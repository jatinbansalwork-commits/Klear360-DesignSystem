...
With this RFC, we want to **establish** certain **rules** (*tokens*) through which we can build predictable motion for our components and **evaluate** **multiple ways** through which we can **use these rules** to build motion within Klear360.
...
#### Tokens - Delay
We will be storing these tokens in `klear360/src/tokens/global/motion.ts` as a `number` of milliseconds.

Here is the **list of tokens** that we will store in Klear360 for `Delay`:
```js
delay: {
  '2xshort': 70,
  xshort: 120,
...
Here is the **list of tokens** that we will store in Klear360 for `Duration`:
```js
duration: {
  quick1: 70,
  quick2: 150,
...
- We will be storing **Easing tokens** in `klear360/src/tokens/global/motion.ts` as a `cubic-bezier()` function.
...
#### Conclusion
- Since we want to leverage all the above mentioned benefits of **we will be using React Native Reanimated** for our animations on Klear360.
...
- Klear360's developers can use this RFC as a guideline to add motion to their components
