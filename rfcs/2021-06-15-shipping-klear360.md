...
- [Detailed Design](#detailed-design)
  ...
  - [How will the consumers import things from klear360?](#how-will-the-consumers-import-things-from-klear360)
...
We can't ship the raw code to the consumers so we need to ship/publish klear360. But to make it publishable there are certain things we need to keep in mind and answer relevant questions.

# Basic Example
We should be able to install and use klear360 in our code without any overhead.
```js
import { Button, Text } from '@klear/klear360/components'
import { overrideTheme, paymentTheme } from '@klear/klear360/theme'
import { getColorScheme, useColorScheme } from '@klear/klear360/utils'
```
...
## How will the consumers import things from klear360?
There are multiple ways that consumers can import components, themes, types, utils from klear360. Let's look into it one by one

### 1. `import` individual components
```js
/* multi-package */
import Button from '@klear-klear360/Button'
import Text from '@klear-klear360/Text'
import overrideTheme from '@klear-klear360/overrideTheme'
import paymentTheme from '@klear-klear360/paymentTheme'
import getColorScheme from '@klear-klear360/getColorScheme'
import useColorScheme from '@klear-klear360/useColorScheme'
```

#### Pros
As maintainers of Klear360, we don't need to maintain a top level re-exports. Just write the component and be done with it.

```js
// example: top level re-exports "packages/klear360/index.ts"
...
```js
/* mono-package */
import { Button, Text, overrideTheme, paymentTheme, getColorScheme, useColorScheme } from '@klear/klear360'
```
...
>It can be automated with scripts but not recommended because as maintainer we need to pick consciously what to re-export as there might be some internal things which we might not want to re-export
```js
// packages/klear360/src/components/index.ts
export * from 'src/components/Button'
export * from 'src/components/Text'

// packages/klear360/src/tokens/theme/index.ts
...
// packages/klear360/src/utils/index.ts
...
import Button from '@klear/klear360/components/Button'
import Text from '@klear/klear360/components/Text'
import paymentTheme from '@klear/klear360/theme/paymentTheme'
```
...
```js
// package.json.name: "@klear-klear360/button"
import Button from '@klear-klear360/button'

// package.json.name: "@klear-klear360/text"
import Text from '@klear-klear360/text'

// package.json.name: "@klear-klear360/tokens"
import { overrideTheme, paymentTheme } from '@klear-klear360/tokens'
```

### 2. Mono-package
We can have mono package i.e only one `package.json` for whole klear360
```js
// package.json.name: "@klear/klear360"
import { Button, Text } from '@klear/klear360/components'
import { overrideTheme, paymentTheme } from '@klear/klear360/tokens'
```
...
Payment and Banking teams both start using klear360 at 1.0.0(assuming we publish a mono package).

Now, we add more features to klear360 and bump the version to 1.0.2
...
* Would we like teams to buy into klear360 as a whole and upgrade versions regularly? Yes, since there's no performance implication as such of mono-package. If teams are not regularly upgrading to newer versions then we'll figure out a way to solve that problem in a different way.
...
1. We can't use library with the script tag in html(umd bundle).
    ```html
    <script src="https://unpkg.com/@klear/klear360@0.1.1/index.js" crossorigin></script>
    ```
2. Consumers are allowed to import anything from anywhere.
    ...
    // this is allowed too
    import Button from '@klear/klear360/_SomeInternalButton'
    ```
...
For example, our bundles `components`, `tokens`, `utils` are generated inside `klear360/build` directory so if our consumers need to consume our packages then they need to do
```js
import {ThemeProvider, useTheme} from '@klear/klear360/build/components';
import {paymentTheme, globalColors} from '@klear/klear360/build/tokens';
```
...
```js
import {ThemeProvider, useTheme} from '@klear/klear360/components';
import {paymentTheme, globalColors} from '@klear/klear360/tokens';
```
