...
This document outlines few guidelines for writing cross platform types internal to klear360

Currently Klear360 uses TypeScript 4.7 to provide better DX to platform specific consumers like `web` or `native`
...
- Provide some standard type utilities to help write cross platform types for klear360 developers
...
With this system in place, klear360 developers have to be bit more deligent while writing types.
...
// klear360 internal component type:
type ButtonProps = {
  onClick?: Platform.Select<{
    native: (event: GestureResponderEvent) => void;
    web: (event: React.MouseEvent<HTMLButtonElement>) => void;
...
> **Note**
> Okay, but klear360 developers have to make sure that all types work in both platforms.
