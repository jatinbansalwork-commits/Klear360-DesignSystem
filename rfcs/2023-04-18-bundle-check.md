...
```
- We will need to add a github action to run bundlemon on every PR. This can be a part of `klear360-validate`(for PR) + `release`(for master push) workflow or a separate `bundle-check` workflow(that triggers on PR & master push).
...
- Explored internal klear projects.
