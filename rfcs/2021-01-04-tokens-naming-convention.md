...
A theme may eventually require `light`, `dark` modes. PG might require a `light` and `dark` mode but their theme is different from Klear X. Similarly, Klear X may require `dark` and `light` mode but their dark and light could be different from any other product's theme
## Storing and accessing tokens

### Where to store tokens?
#### Storing theme tokens
The theme tokens shall we created under `packages/klear360/tokens/theme`
#### Storing component tokens
The component tokens can be created in 2 ways:
1. Co-located inside the component file.
2. Create a token file under the component's directory i.e `packages/klear360/components/Button/tokens.ts`.
3. Create component tokens under `packages/klear360/tokens`.
...
* This is being built in isolation(though merged into master regularly as we follow Trunk based development) and hence nothing will break on the projects using klear360 design system directly from git commit.
...
- I'm still not able to visualise if creating component tokens under `packages/klear360/tokens` makes sense at all?
