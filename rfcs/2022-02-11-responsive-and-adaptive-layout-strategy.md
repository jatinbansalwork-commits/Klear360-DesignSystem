...
- [Detailed Design](#detailed-design)
  ...
  - [How will we actually implement the approaches discussed above?](#how-will-we-actually-implement-the-approaches-discussed-above)
    - [On Klear360 Code Side](#on-klear360-code-side)
    - [On Consumer Code Side](#on-consumer-code-side)
    - [On Klear360 Design Side](#on-klear360-design-side)
...
We'll basically define the approach and strategy we'll be taking in Klear360 to build components that will address the concerns of rendering based on different screen sizes.
...
- Adaptive works to detect the screen size and load the appropriate layout for it – generally you would design an adaptive site for six common screen widths(confirm with Priya once on exact sizes we are doing and update):
...
After doing some research and aligning it with our uses cases at Klear, I propose following things at a broader level.
...
For this to work, on the consumer side nothing has to be done explicitly, as far as the consumers have wrapped their app inside `<Klear360Provider><ConsumerApp></Klear360Provider>`, the components which will handle responsiveness internally will automatically listen to the screen size changes and scale automatically based on the available space
...
```jsx
import { Stack, Card, Title } from '@klear/klear360/components';
...
- Consider, if we want to implement a layout similar to the below mockup where we have 2 different search inputs for different screens of the same web app.
  ...
  ```jsx
  import { LargeSearchInput, SmallSearchInput } from '@klear/klear360/components';
  ...
  const LargeSearchInput = React.lazy(() => import('@klear/klear360/components'));
  const SmallSearchInput = React.lazy(() => import('@klear/klear360/components'));
  ...
  const LargeSearchInput = loadable(() => import('@klear/klear360/components'));
  const SmallSearchInput = loadable(() => import('@klear/klear360/components'));
...
- Consider, another example where we want to show and hide bunch of components based on different screen sizes.
  ...
  ```jsx
  import loadable from '@loadable/component';
  import { Stack, Card, Title, Hidden, Avatar, Divider } from '@klear/klear360/components';

  const DesktopNavBar = loadable(() => import('@klear/klear360/components'));
  const MobileNavBar = loadable(() => import('@klear/klear360/components'));
  const BottomNavBar = loadable(() => import('@klear/klear360/components'));
  ...
  > The `isMobile` logic can be be abstracted out withing Klear360 or left upto consumers, there are pros and cons to both the approaches hence we shall evaluate this case by case basis and it's out of scope of this document.

### On Klear360 Design Side
...
1. Why is there no _Automatic_ Adaptive?
   ...
   A valid use-case here could be a `Dropdown` component where we _know_(\*assuming it here) that we always want the dropdown to be almost fullscreen on mobile but a relatively small pop-over menu on desktop. Consumers can just import `Dropdown` and klear360 can internally render the `MobileDropdown` or `DesktopDropdown` based on the current screen size and take away the complexity of adding any logical checks for the consumers.

2. How do we communicate to consumers that a component they are using is Responsive or Adaptive?

   As a consumer of klear360, how would I know if a component **ComponentA** is automatically responsive or I need to pass it some props for it to become responsive or is it adaptive and I need to import another **SmallComponentA** and add `isMobile` check to be able to render it properly on a mobile device?
