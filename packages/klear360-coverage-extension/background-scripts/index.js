...
import { calculateKlear360Coverage } from './klear360-coverage-script';
  ...
  if (message.action === 'executeScript') {
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      const response = await chrome.scripting.executeScript({
        ...
        func: calculateKlear360Coverage,
        args: [message.shouldHighlightNodes, message.includeNavbars],
      });
      console.log('response', response);
      chrome.runtime.sendMessage({ action: 'klear360-coverage', coverage: response[0].result });
    });
  }
