/**
 * Generates `packages/klear360-core/src/tokens/theme.css` from the token sources.
 *
 * A Figma token push rewrites the token TS files (`colors.ts`, `klear360Theme.ts`) but historically
 ...
 *
 * Scope: klear360Theme only. `theme.css` encodes a single theme (light `:root` + dark block + mobile
 * `@media`). Theme choice (klear360 vs neutral) is a JS prop with no DOM attribute, so a static neutral
 ...
 * `@layer klear360;` (line 4, no brace) is intentionally NOT this — the tail is the braced `@layer`.
 */
const TAIL_MARKER = '@layer klear360 {';

const DARK_SELECTOR_BLOCK = `/* Dark Mode
 * Transition period: both \`body[data-theme='dark']\` (legacy) and
 * \`[data-klear360-color-scheme='dark']\` (new scoped attribute) are supported.
 * \`:root[data-klear360-color-scheme]\` is a compound selector (no space) so it
 * matches \`<html>\` itself — where Klear360Provider sets the attribute — and
 ...
 * migrate to \`data-klear360-color-scheme\`. */
:root body[data-theme='dark'],
:root[data-klear360-color-scheme='dark'] {`;
...
export const generateThemeCSS = (): string => {
  const { colors, typography } = klear360Theme;

  const head: string[] = [
    '/* Klear360 Design System CSS Variables */',
  ...
  // `theme-css-layers.test.ts` asserts on exactly (e.g. unindented `@layer klear360 {\n/* ... */`).
