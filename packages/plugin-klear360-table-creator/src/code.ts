...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'create-table') {
    ...
    // Define color keys for Klear360 colors
    const KLEAR360_COLOR_KEYS = {
    ...
    try {
      const variableEntries = Object.entries(KLEAR360_COLOR_KEYS) as [
      ...
      console.error('Failed to import Klear360 color variables', error);
      figma.notify(
        `Error loading Klear360 color variables: ${
    ...
    frameTable.fills = [
      createVariableBoundPaint(KLEAR360_COLOR_KEYS['background.gray.intense'].id),
    ];
    ...
    try {
      ...
      if (!tableHeaderCellText || !tableBodyCellText) {
        figma.notify(
          'Error: Could not load components from library. Maybe the Klear360 library is not imported. Please check your assets panel.',
        );
        figma.closePlugin();
        return;
      }
