# Klear360 Design Tokens

Token documentation covers the design tokens that power Klear360's visual system — spacing, color, typography, motion, and more.
...
Klear360 uses a **two-tier token architecture** to maintain a consistent design language across web and React Native platforms.
...
```ts
ThemeTokens = {
  name: string;           // "klear360Theme" or custom
  border: { radius, width }
...
Klear360 components accept string tokens for spacing, color, and other props:
...
| **Platform-aware** | Typography and elevation have platform-specific values (desktop/mobile, web/native). Klear360 components handle this automatically. |
...
## Klear360Provider Setup

Every Klear360 application must be wrapped with `Klear360Provider`:
...
### Nested Klear360Provider for mixed modes

If a section needs a different color scheme (e.g., dark section on a light page), wrap it in another `Klear360Provider`:
...
When `colorScheme="light"`, Klear360 resolves `theme.colors` from `theme.colors.onLight`. When `colorScheme="dark"`, it resolves from `theme.colors.onDark`. Consumers never need to specify `onLight`/`onDark` — they access `theme.colors.surface.*` and the correct values are provided automatically.
...
### In Klear360 component props (string tokens)
...
```ts
import { useTheme } from '@klear/klear360/components';
...
For non-Klear brands, use `createTheme()` to generate a custom theme from a brand color. See the **WhiteLabelling** documentation for details on `createTheme()` API and usage.
...
Klear360's color system has two layers:
...
|---|---|---|---|
| Text | Inter, Roboto, Arial | Inter | Inter |
| Heading | Inter, Roboto, Arial | Inter | Inter |
| Code | Roboto Mono | Roboto Mono | Roboto Mono |
...
### Using Klear360 typography components (recommended)

Klear360 provides `Text`, `Heading`, `Display`, and `Code` components that automatically handle:
...
Using Klear360 typography components is strongly preferred over manually accessing typography tokens, as they handle platform-specific sizing automatically.
...
| Category | Web | React Native (iOS) | React Native (Android) |
|---|---|---|---|
| Text | Inter, Roboto (with fallbacks) | Inter | Inter |
| Heading | Inter, Roboto (with fallbacks) | Inter | Inter |
| Code | Roboto Mono + mono fallback stack | Roboto Mono | Roboto Mono |
...
Klear360 `Text`, `Heading`, and `Code` components automatically choose the correct font family for the context and platform.
...
- React Native uses `Roboto Mono` for the `code` font on both iOS and Android.
...
Klear360 uses a 12-step spacing scale built on a 4px base grid. Values are in pixels.
...
Klear360 uses a mobile-first responsive system with 6 breakpoints.
...
## Responsive Props in Klear360 Components
...
### In Klear360 animation components

Klear360 provides animation components (`Fade`, `Slide`, `Move`, `Scale`, `Stagger`, `Morph`) that use motion tokens internally. Prefer these over manual CSS transitions when possible.
...
Elevation tokens support `onLight`/`onDark` mode variants, resolved automatically by `Klear360Provider`.
...
```jsx
import { Elevate } from '@klear/klear360/components';
...
Klear360 currently exposes a backdrop blur scale with 3 levels:
...
Size tokens are a global px scale used across Klear360 internals for fixed dimensions.
