...
Several Klear360 components let a user choose from a set of options, and a few of them look visually similar (most notably `Tabs` and `SegmentedControl`). Pick the component based on **how the selection behaves**, not just how it looks.

Use this decision tree, then confirm against the quick-reference and the specific component's own "When to Use" section (via `get_klear360_component_docs`).
