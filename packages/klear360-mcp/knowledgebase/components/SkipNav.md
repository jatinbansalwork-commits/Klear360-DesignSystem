...
- Ensure the `id` prop on `SkipNavLink` matches the `id` on `SkipNavContent` — they default to `"klear360-skip-nav"`.
...
```tsx
import { SkipNavLink, SkipNavContent, Box, Link, Text, Heading } from '@klear/klear360/components';
