  ...
  return (
    <Box padding="spacing.5">
      ...
      <ButtonGroup variant="primary" size="medium">
        <Button icon={PlusIcon} onClick={() => console.log('Create new transfer')}>
          Payout
        </Button>
        <Dropdown>
          <DropdownButton icon={ChevronDownIcon} accessibilityLabel="More transfer options" />
          <DropdownOverlay defaultPlacement="bottom-end">
            <ActionList>
              <ActionListItem
                title="Bulk Payout"
                value="bulk-transfer"
                onClick={() => handleActionSelect('bulk-transfer')}
              />
              <ActionListItem
