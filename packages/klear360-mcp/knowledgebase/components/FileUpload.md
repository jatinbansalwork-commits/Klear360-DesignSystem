...
type FileUploadProps = {
   ...
   */
  onChange?: (info: { fileList: Klear360FileList }) => void;

  /**
   * Callback when files are dropped on the dropzone
   */
  onDrop?: (info: { fileList: Klear360FileList }) => void;

  /**
   * Callback when the preview button is clicked
   */
  onPreview?: (info: { file: Klear360File }) => void;

  /**
   * Callback when the remove button is clicked
   */
  onRemove?: (info: { file: Klear360File; fileList: Klear360FileList }) => void;

  /**
   * Callback when file upload is started
   */
  onUpload?: (info: { file: Klear360File; fileList: Klear360FileList }) => void;
...
import { FileUpload, TextInput, Button, Box } from '@klear/klear360/components';
...
```tsx
import React, { useState } from 'react';
import { FileUpload, Box } from '@klear/klear360/components';
import type { Klear360File, Klear360FileList } from '@klear/klear360/components';

const MultipleFileUploadExample = () => {
  const [fileList, setFileList] = useState<Klear360FileList>([]);

  // Simulate file upload with progress
  const simulateUpload = (file: Klear360File) => {
