...
type UseToastReturn = {
  toasts: Klear360Toast[]; // Currently active toasts
...
- Place `<ToastContainer />` once at the root of your app, outside nested `Klear360Provider` instances.
...
- Don't render `ToastContainer` inside `Klear360Provider` children if you have nested providers — place it at the app root.
...
```tsx
import { ToastContainer, useToast, Box, Button } from '@klear/klear360/components';
...
```tsx
import { ToastContainer, useToast, Box, Button, Heading, Text } from '@klear/klear360/components';
