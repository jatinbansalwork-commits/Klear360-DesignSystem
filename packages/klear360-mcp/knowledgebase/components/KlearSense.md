...
- `preloadKlearSenseAssets(preset)` should be called and awaited before mounting the component with that preset to avoid visible loading artifacts.
...
The following types represent the props that the KlearSense component accepts.

```typescript
type KlearSensePreset = 'default' | 'zoomed' | 'bottomWave' | 'rippleWave' | 'circleSlideUp';

type KlearSenseProps = {
...
- Use `KlearSense` for immersive, branded visual moments: success screens, hero sections, login pages, and loading states.
- Use presets (`'default'`, `'zoomed'`, `'bottomWave'`, `'rippleWave'`, `'circleSlideUp'`) for quick setup; customize with individual props as needed.
- Call `preloadKlearSenseAssets(preset)` before mounting to avoid visible frame skipping on first render.
- Use `edgeFeather` for vignetting when overlaying text or UI on top of the animation.
- Layer `KlearSenseGradient` on top as a foreground icon overlay for the full Spark Animation pattern.

**Don't**

- Don't use `KlearSense` on React Native — it requires WebGL and is web-only.
...
- Don't use `KlearSense` for simple transitions or button feedback — use standard CSS/framer-motion animations for lightweight effects.
- Don't pass hex/CSS colors to `backgroundColor` — it expects normalized RGB `[0-1, 0-1, 0-1]`.

## Example

Here are examples demonstrating various ways to use the KlearSense component:
...
import { KlearSense, Box } from '@klear/klear360/components';

const DefaultExample = () => {
  return (
    <Box width="100%" height="100vh">
      <KlearSense width="100%" height="100%" />
    </Box>
  );
};
  ...
  return (
    <Box display="flex" flexDirection="column" gap="spacing.6">
      ...
      <Box width="100%" height="400px">
        <KlearSense width="100%" height="100%" preset="zoomed" />
      </Box>

      {/* Bottom wave — decorative footer/header strip */}
      <Box width="100%" height="250px">
        <KlearSense width="100%" height="100%" preset="bottomWave" />
      </Box>
...
Uses `preloadKlearSenseAssets` to preload assets before mounting, combined with a success UI card and framer-motion animations.
