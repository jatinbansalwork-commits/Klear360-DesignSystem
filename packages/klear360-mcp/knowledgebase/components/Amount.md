...
The Amount component is used to display currency values with proper formatting. It shows small amounts of color-coded metadata, which are ideal for getting user attention. This component only displays the provided value in the specified currency with the formatting capabilities enabled by @klear/i18n-react, it does not perform any currency conversion.
...
type AmountCommonProps = {
   ...
   * Note: Currency symbol and code is determined by the locale set in user's browser or set via @klear/i18n-react library.
...
```tsx
import { Amount } from '@klear/klear360/components';
