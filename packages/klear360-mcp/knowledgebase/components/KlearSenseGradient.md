...
KlearSenseGradient renders an animated WebGL fluid gradient clipped to an SVG mask shape. Pass SVG elements (path, g, circle, etc.) as children to define the visible area — the gradient fills those shapes. Works well with framer-motion SVG variants (`motion.path`, `motion.g`) for animated masks. Commonly used alongside KlearSense for icon/logo treatments in success and loading states.
...
The following types represent the props that the KlearSenseGradient component accepts.

```ts
type KlearSenseGradientProps = {
  /**
...
- Use `KlearSenseGradient` for animated gradient effects masked to SVG icon/logo shapes.
...
- Layer over `KlearSense` as a foreground icon overlay for the full Spark Animation pattern.
- Combine with framer-motion SVG variants (`motion.path`, `motion.g`) for animated mask effects.

**Don't**

- Don't use `KlearSenseGradient` on React Native — it requires WebGL and is web-only.
...
Here are examples demonstrating various ways to use the KlearSenseGradient component:
...
import { KlearSenseGradient } from '@klear/klear360/components';

const BasicExample = () => {
  return (
    <KlearSenseGradient size={100} viewBox="0 0 24 24">
...
### Combined with KlearSense

A common pattern — KlearSense provides the background animation while KlearSenseGradient renders an animated gradient icon on top.
