...
import { SKILL_VERSION, KLEAR360_SKILL_FILE_PATH, SKILL_VERSION_STRING } from '../tokens.js';
...
describe('skillUtils', () => {
  describe('shouldCreateOrUpdateSkill (file-system path, skipLocalSkillChecks = false)', () => {
    ...
    it('should return undefined when skill file exists and is up to date', () => {
      ...
      const result = shouldCreateOrUpdateSkill('0', tmpDir, false, 'get_klear360_component_docs');
    ...
    it('should return undefined when skill file contains the actual SKILL.md template content (regression: quote mismatch)', () => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'klear360-skill-test-'));
      // Simulate what createKlear360Skill writes — the real template content
      makeSkillFile(tmpDir, fs.readFileSync(KLEAR360_SKILL_FILE_PATH, 'utf8'));
