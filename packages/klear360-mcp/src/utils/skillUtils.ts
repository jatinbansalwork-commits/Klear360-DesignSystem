...
import { createKlear360SkillToolName } from '../tools/createKlear360Skill.js';
...
const bashScriptContent = `
  ...
  BASE_URL="https://raw.githubusercontent.com/jatinbansalwork-commits/Klear360/main/packages/klear360-mcp/skillTemplate"
  # Create directories
  mkdir -p "$SKILL_DIR"
  mkdir -p "$REFS_DIR"
...
3. This will create the klear360 skill at \`.agents/skills/ui-code-guidelines/SKILL.md\` and a symlink at \`.claude/skills/ui-code-guidelines\`

Run the following bash script to create the klear360 skill:
  ...
  if (!skipLocalSkillChecks && toolName) {
    return handleError({
      ...
      mcpErrorMessage: isMissing
        ? `Klear360 skill does not exist. Call \`${createKlear360SkillToolName}\` first.`
        : `Klear360 skill is outdated. Call \`${createKlear360SkillToolName}\` first to update the skill`,
    });
  }
    ...
    content: [
      {
        type: 'text',
        text: isMissing
          ? `Klear360 skill does not exist. Create it using the following instructions: \n\n
          ${skillCreationInstructions({
            currentProjectRootDirectory,
          })}`
          : `Klear360 skill is outdated. Expected version: ${SKILL_VERSION}. Current version: ${skillVersion}. Update it using the following instructions: \n\n
          ${skillCreationInstructions({
            currentProjectRootDirectory,
          })}`,
      },
