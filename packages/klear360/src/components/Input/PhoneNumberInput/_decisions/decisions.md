...
- [Figma - PhoneNumberInput](https://www.figma.com/file/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=83906%3A15428&mode=dev)
...
```jsx
import { PhoneNumberInput } from '@klear/klear360';
...
And for handling internationalization of phone numbers, we will use `@klear/i18n`.
...
- The country selector will be accessible via keyboard navigation and be composed with klear360's Dropdown component.
...
**Participants:** Design, Engineering, Security
...
1. i18n team has exposed a function called `getFlagOfCountry` which will return the flag svg via the [flagpedia CDN](https://flagpedia.net/download/api), we will have to render it as an image.
...
**Action Items:**
1. Design will change the design to use the flags from the flagpedia CDN.
2. Design will update the design to show the format in placeholder and have non-formatted basic input field value.
3. Engineering will use the `getFlagOfCountry` function from i18n library.
4. Security to get approval on the flagpedia CDN.
