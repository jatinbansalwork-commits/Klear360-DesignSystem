...
const _TextInput: React.ForwardRefRenderFunction<Klear360ElementRef, TextInputProps> = (
  {
    label,
    accessibilityLabel,
    labelPosition = 'top',
  ...
  const textInputRef = React.useRef<Klear360ElementRefWithValue>(null);
  ...
  if (__DEV__) {
    if (format) {
      ...
      if (hasAlphanumeric) {
        throw new Error(
          `[Klear360: TextInput] Invalid format "${format}". Only # and special characters allowed, no letters/numbers.`,
        );
      }
    }
  }
