...
describe('<TextInput />', () => {
  ...
  it('should throw error when both value and defaultValue are passed', () => {
    ...
    expect(() =>
      renderWithTheme(<TextInput label="Enter name" defaultValue="Jordan" value="Jordan Hale" />),
    ).toThrow(
  ...
  it(`should add tags in uncontrolled API`, async () => {
    ...
    await userEvent.type(input, klear360Email);
    await userEvent.type(input, ',');
    expect(getTag(klear360Email)).toBeVisible();
    ...
    await userEvent.click(getTag(klear360Email));
    await waitFor(() => expect(getTag(klear360Email)).not.toBeVisible());
  });
