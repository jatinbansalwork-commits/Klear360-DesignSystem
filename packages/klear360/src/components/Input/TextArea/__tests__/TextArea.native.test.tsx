...
const klear360Email = 'klear360@gmail.com';
...
describe('<TextArea />', () => {
  ...
  it('should set value as a controlled input', () => {
    const valueInitial = 'Sam';
    const valueFinal = 'Sam Ortiz';
  ...
  it('should throw error when both value and defaultValue are passed', () => {
    ...
    expect(() =>
      renderWithTheme(<TextArea label="Enter name" defaultValue="Jordan" value="Jordan Hale" />),
    ).toThrow(
      `[Klear360: Input]: Either 'value' or 'defaultValue' shall be passed. This decides if the input field is controlled or uncontrolled`,
    );
  ...
  it(`should add tags in uncontrolled API`, () => {
    ...
    expect(getTag(klear360Email)).toBeUndefined();
    fireEvent.changeText(input, klear360Email);
    fireEvent(input, 'onKeyPress', { nativeEvent: { key: ',' } });
    expect(getTag(klear360Email)).toBeOnTheScreen();
    expect(tagChangeCallback).toBeCalledWith({ tags: [klear360Email] });

    expect(getTag(tagEmail)).toBeUndefined();
    fireEvent.changeText(input, tagEmail);
    fireEvent(input, 'onKeyPress', { nativeEvent: { key: ',' } });
    expect(tagChangeCallback).toBeCalledWith({ tags: [klear360Email, tagEmail] });
  });
  ...
  it(`should add tags in controlled API`, () => {
    ...
    const Example = (): React.ReactElement => {
      ...
      return (
        <>
          ...
          <Button
            onClick={() => {
              setTags([...tags, 'alex@klear.com', 'riley@klear.com']);
            }}
          >
            Add More
          </Button>
    ...
    expect(getTag(klear360Email)).toBeOnTheScreen();
    expect(getTag('alex@klear.com')).toBeOnTheScreen();
    expect(getTag('riley@klear.com')).toBeOnTheScreen();
  });
