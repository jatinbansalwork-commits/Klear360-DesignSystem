...
The "no match-mode dropdowns" statement above is about *match-mode* pickers — an operator selector like contains/starts-with/equals, or an AND/OR constraint builder. It was never a statement about *value* pickers. Selecting one or more known values for a column (e.g. Status = `Pending` or `Pending`/`Completed`) is a distinct, real gap - a competitive audit need identified it the same way the original text-filtering gap was identified - so it's been added as an opt-in per-column addition, not a change to the default experience.
