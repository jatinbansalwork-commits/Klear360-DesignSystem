...
- All the icons on klear360 will be rendered as SVG.
- Web will use the native `svg` elements to render the icons.
- Native will use `react-native-svg` to render the icons.
- In the past (klear360-old), we used to create 2 files per icon - One for web and one for native (`WifiOn.web.js` & `WifiOn.native.js`).
...
- To achieve this, we'll create custom Svg components like `Svg`, `Path`, `G`, `Rect`, etc. within klear360 which will have their own platform specific implementations like `Svg.native.tsx` & `Svg.web.tsx` that would use the correct platform specific components to render the icon.
- These will be internal components and won't be exported to our consumers
- We would use these components to build our Klear360 Icons which would then be exported
...
- After auditing components from `klear360-old`, we have decided to create the following Svg components (we can add more components & props later if we get a valid use-case):
  - Svg
  - Path
  - G
  - Rect
...
- We will store all our icons within `components/Icons/*` directory and make them available to our consumers like this:
  ```jsx
  import { CreditCardIcon } from '@klear/klear360/components';
  ```
...
- For now, we will include a few important icons in klear360. We will add more icons later as we get more use-cases. You can find the list of all supported klear360 icons in the Klear360 Figma file.
- For icons that are not supported by Klear360 (not available on Klear360's Figma), consumers would need to create their own custom icons using html svg & `react-native-svg` since we're not exporting `_Svg` to our consumers.
