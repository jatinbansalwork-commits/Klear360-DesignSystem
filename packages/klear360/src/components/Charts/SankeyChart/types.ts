...
export type SankeyDataNode = {
  ...
  name: string;
  /** Optional typed Klear360 color token override, e.g. 'data.background.categorical.blue.moderate' */
...
export type ChartSankeyWrapperProps = {
  ...
  /** Override all node bar colors with a single Klear360 token */
  nodeColorOverride?: ChartsCategoricalColorToken;
  /** Override all link ribbon colors with a single Klear360 token */
...
export type ChartSankeyProps = {
  ...
  showLabels?: boolean;
  /**
   * When true (default), labels render as Klear360-styled chips with value + percentage.
