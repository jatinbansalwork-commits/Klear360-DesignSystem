...
describe('SankeyChart — rendering', () => {
  ...
  it('sets data-klear360-component attribute on wrapper for analytics', () => {
    const { container } = renderSankey();
    expect(container.querySelector('[data-klear360-component="ChartSankeyWrapper"]')).toBeTruthy();
  });
...
describe('SankeyChart — interactivity', () => {
  ...
  it('does not show tooltip when showTooltip is false', () => {
    ...
    const tooltipContent = container.querySelector(
      '[data-klear360-component="ChartSankeyWrapper"] > div > div[style*="position: absolute"]',
    );
...
describe('SankeyChart — color token resolution', () => {
  it('applies nodeColorOverride to all node rects with a single resolved color', () => {
    ...
    // The resolved fill should be a real CSS color (not a Klear360 token string like 'data.background...')
    const [fill] = fills;
...
describe('SankeyChart — edge cases', () => {
  ...
  it('throws a Klear360 error when ChartSankey is rendered outside ChartSankeyWrapper', () => {
