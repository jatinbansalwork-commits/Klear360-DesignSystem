...
- Figma Design[ Line](https://www.figma.com/design/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=92678-188716&p=f&m=dev), [Area](https://www.figma.com/design/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=92678-188716&p=f&m=dev), [Bar](https://www.figma.com/design/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=92678-188719&p=f&m=dev), [Donut](https://www.figma.com/design/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=92678-188718&p=f&m=dev) Charts.
...
```ts
// Import statement
import { AreaChart } from '@klear/klear360/charts';
const areaConfig = [
  {
    dataKey: 'teamA',
...
| `color`             | `Klear360ColorToken`                                                 | ❌       | Auto-assigned | Color token for the line (automatically assigned from palette if not provided)                                                                                            |
      ...
      <ChartLineWrapper data={forecastData}>
          <ChartCartesianGrid/>
            <ChartXAxis dataKey="date" />
              <ChartYAxis />
              // This would be a custom tooltip from klear360
              <ChartTooltip />
              <ChartLegend />
              <ChartLine
                dataKey="historical"
...
| `color`             | `Klear360ColorToken`                                                 | ❌       | Auto-assigned | Color token for the area fill (automatically assigned from palette if not provided)                                                                                                                                               |
...
| `color`     | `Klear360ColorToken`               | ❌       | Auto-assigned   | Color token for the bar (maps to `fill` prop internally)                               |
...
// Custom Label Bar Chart (note: high chance we might not need this.)
    <ChartBarWrapper data={chartData}>
        ...
        <ChartYAxis />
        <ChartBar dataKey="seriesA" name="Series A" color="surface.action.primary.default">
            {/* Klear360 will provide a styled LabelList component */}
...
| `nodeColorOverride` | `ChartsCategoricalColorToken` | ❌       | —               | Override all node bar colors with a single Klear360 token                          |
| `linkColorOverride` | `ChartsCategoricalColorToken` | ❌       | —               | Override all link ribbon colors with a single Klear360 token                       |
| `testID`            | `string`                      | ❌       | —               | `data-testid` for the wrapper element                                           |
| `...BoxProps`       | `BoxProps`                    | ❌       | `height="100%"` | All Klear360 Box layout props — use `height` / `width` to size the chart container |
...
| `showLabelChip`  | `boolean`                                              | ❌       | `true`                   | Render labels as Klear360 chip cards; `false` renders plain SVG text |
...
```tsx
import { ChartSankeyWrapper, ChartSankey } from '@klear/klear360/components';
...
| `color` | `Klear360ColorToken` | ❌       | none        | Color token for the reference line                    |
...
We will not support sequential colors for line charts. A limited, curated palette of theme colors will be available. A `color` prop will be available in the line configuration. If it is not provided, we will automatically assign a color from the predefined palette. [Rama Krushna Behera](mailto:rama.behera@klear.com) will confirm the final color palette.

- Tooltip

We might need to make custom tooltip. Like ChartsToolTip. Since the tooltip would be different then our klear360's tooltip. (We are awaiting Designs on this.)

- Klear360ChartDots - Awaiting Designs on this.
