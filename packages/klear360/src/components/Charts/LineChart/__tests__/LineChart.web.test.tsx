...
describe('<LineChart />', () => {
  ...
  it('should not render a dashed bridge path by default (hard gap)', () => {
    ...
    expect(container.querySelectorAll('.klear360-null-bridge-layer path')).toHaveLength(0);
  });
  ...
  it('should render a curved dashed bridge path when connectNullsStyle is "dashed"', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll('.klear360-null-bridge-layer path')).toHaveLength(1);
    });
    const bridgePath = container.querySelector('.klear360-null-bridge-layer path')!;
...
describe('<ChartReferenceBand />', () => {
  ...
  it('should paint a filled band path between the bounds', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll('.klear360-reference-band-layer path')).toHaveLength(1);
    });
    const bandPath = container.querySelector('.klear360-reference-band-layer path')!;
  ...
  it('should not render a band layer when no ChartReferenceBand is present', () => {
    ...
    expect(container.querySelectorAll('.klear360-reference-band-layer path')).toHaveLength(0);
  });
...
describe('LineChart per-line reference bands (range on ChartLine)', () => {
  ...
  it('should paint one band per line that declares a range', async () => {
    ...
    await waitFor(() => {
      ...
      expect(container.querySelectorAll('.klear360-reference-band-layer path').length).toBe(2);
    });
    const bandPaths = container.querySelectorAll('.klear360-reference-band-layer path');
  ...
  it('should only paint bands for lines that declare a range', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll('.klear360-reference-band-layer path').length).toBe(1);
    });
  });
