...
- [Figma - Side Navigation](https://www.figma.com/file/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=87921-138309&mode=dev)
...
2. Component from Klear360 is better to kept independent of backend API schema since that schema can change, update, etc and shouldn't be blocked on klear360.
...
| icon        | Klear360's Icon Component                                                                                                       | IconComponent                 |                                                              |
...
| trailing  | Trailing Slot of Item. Render Switch, or any other klear360 component here              | JSX          |                   |
| tooltip   | Object with props that are forwarded to tooltip                                      | TooltipProps | undefined         |
...
2. We should use Klear360's `SkipNav` utility to provide option of skipping nav and going to content
...
- #### Why `isActive` prop? Why not handle active state internally
  ...
  Earlier we started the API by saying that marking of the link as active based on route will be handled internally in klear360 component. There are some implemetation constraints when we go with this approach of handling active link internally

  1. We don't have access to hooks like `useLocation` or utilities like `matchPath` inside Klear360 since klear360 is library indepedent of the routing logic
...
- Product Questions Discussion - Slack Thread
- #### Should URLs follow the nesting defined by navigation?
  ...
  - Conclusion:
    - While we discussed this internally (in engineering), it becomes out of scope for Klear360 since [we can't handle active state internally anyways](#why-isactive-prop-why-not-handle-active-state-internally)
