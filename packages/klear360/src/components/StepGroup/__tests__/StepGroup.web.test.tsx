...
describe('<StepGroup />', () => {
  ...
  it('should render as anchor tag when href is passed', () => {
    const { getByRole } = renderWithTheme(
      <StepGroup>
        <StepItem title="Introduction" href="https://klear.com" />
      </StepGroup>,
    );
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toEqual(
          '[Klear360: StepItem]: trailing is not allowed in horizontal StepGroup',
        );
      }
    }
  ...
  it('should throw an error when nested StepGroup is defined', () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toEqual(
          '[Klear360: StepItem]: Nested StepGroup components are not allowed in horizontal orientation',
        );
      }
    }
    console.error = tempConsoleError;
