...
## Design
- [Figma - Skeleton](https://www.figma.com/file/jubmQL9Z8V7881ayUD95ps/Klear360---Payment-Light?node-id=16508%3A258522&mode=dev)
...
To use Skeleton loader with existing klear360 components like Card, consumers can create custom skeleton templates as per their needs and usecases.
...
const App = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardHeaderLeading title="Payment options" subtitle="Secured by Klear" />
...
<details>
  ...
  - Providing `isLoading` prop in Card or for all Klear360 components (where applicable) might not be the most flexible approach.
...
<details>
  ...
  **Inferring Typography components:**
  ...
  ```jsx
  <Heading size="large">
    {isLoading ? <Skeleton /> : "Hello world this is klear360"}
  </Heading>
  ```

  [Demo Codesandbox](https://codesandbox.io/s/klear360-skeleton-text-infer-dim-tmm6dn?file=/App.tsx)
