...
const scrollStepIntoView = async (
  ...
  const contentOffset = (scrollable as any)?.__klear360TourContentOffset ??
  ...
  const wasLocked = (scrollable as any).__klear360TourScrollLocked === true;
  if (wasLocked) {
  ...
  try {
    ...
    (scrollable as any).__klear360TourContentOffset = { x: targetX, y: targetY };
  } finally {
    if (wasLocked) {
...
const useTourScrollLock = (
  ...
  React.useEffect(() => {
    ...
    (scrollable as any).__klear360TourScrollLocked = true;
    (scrollable as any).scrollEnabled = false;

    return () => {
      scrollable.setNativeProps?.({ scrollEnabled: originalScrollEnabled ?? true });
      (scrollable as any).__klear360TourScrollLocked = false;
