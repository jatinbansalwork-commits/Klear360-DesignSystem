...
Design: [Klear360 DSL — FAB v1.0](https://www.figma.com/design/jubmQL9Z8V7881ayUD95ps/Klear360-DSL?node-id=125809-2463)
...
```jsx
import { FloatingActionButton } from '@klear/klear360/components';
import { PlusIcon } from '@klear/klear360/components';
...
**`offset` is a single token rather than per-axis.** It covers the common case, and `StyledPropsKlear360` is applied after the placement styles, so a caller needing asymmetric offsets (for example clearing a `BottomNav`) can pass `bottom="spacing.10"` directly without a second offset API.
