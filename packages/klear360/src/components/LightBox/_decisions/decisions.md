...
```jsx
import { LightBox, LightBoxBody, LightBoxItem } from '@klear/klear360/components';
...
When some items are not images, use the custom mode with `children` + `thumbnail`. Use Klear360's `Preview` component for zoomable document/image rendering.
...
- [Klear360 Preview component](../../Preview/Preview.web.tsx)
