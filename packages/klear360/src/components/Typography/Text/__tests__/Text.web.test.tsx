...
describe('<Text />', () => {
  ...
  it('should throw error when variant is "caption" and size "large" is passed', () => {
    ...
    expect(() =>
      renderWithTheme(
        // TODO: something broke on ts after changing types. Check with Maya on why this is happening
    ...
    ).toThrow(`[Klear360: Text]: size cannot be 'large' when variant is 'caption'`);
  ...
  it('should throw error on invalid as prop', () => {
    ...
    expect(() =>
      ...
      '[Klear360: Text]: Invalid `as` prop value - button. Only p, span, div, abbr, figcaption, cite, q, label are accepted',
    );
  });
