...
describe('<BaseLink />', () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toEqual(
          `[Klear360: BaseLink]: At least one of icon or text is required to render a link.`,
        );
      }
    }
  });
  ...
  it('should handle styles for all interactions', () => {
    ...
    expect(linkButtonText).toHaveStyle({
      color: klear360Theme.colors.onLight.interactive.text.primary.normal,
    });

    // click and focus
    fireEvent.focus(linkButtonText);
    expect(linkButtonText).toHaveStyle({
      color: klear360Theme.colors.onLight.interactive.text.primary.subtle,
    });
