...
import { logger, throwKlear360Error } from '~utils/logger';
...
const makeLoadMoreValue = (parentValue: string | null, posInSet: number): string =>
  `__klear360-tree-load-more__${parentValue ?? '$root'}__${posInSet}`;

const isTreeViewLoadMoreValue = (value: string): boolean =>
  value.startsWith('__klear360-tree-load-more__');
