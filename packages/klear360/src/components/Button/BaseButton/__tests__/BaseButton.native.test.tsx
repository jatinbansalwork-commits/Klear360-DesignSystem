...
describe('<BaseButton />', () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toEqual(
          `[Klear360: BaseButton]: At least one of icon or text is required to render a button.`,
        );
      }
    }
  });
  ...
  it('should throw error if tertiary variant is passed with positive color', () => {
    ...
    expect(() =>
      ...
      `"[Klear360: BaseButton]: Tertiary variant can only be used with color: "primary" or "white" or "transparent" but received "positive""`,
    );
  });
