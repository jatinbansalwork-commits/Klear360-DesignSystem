...
const createMockNativeAttachment = (index: number): Klear360FileList[0] =>
  ({
    id: `native-mock-${Date.now()}-${index}`,
    name: `attachment-${index}.pdf`,
    size: 1024,
    status: 'success',
  } as Klear360FileList[0]);

const handlePlatformFileChange = (
  setFiles: React.Dispatch<React.SetStateAction<Klear360FileList>>,
): ((args: { fileList: Klear360FileList }) => void) => {
  return ({ fileList }) => {
    if (isReactNative()) {
...
const Page = (): React.ReactElement => {
  return (
    ...
    >
      ...
      <Sandbox showConsole>
        {`
        import { ChatInput } from '@klear/klear360/components';
...
export const WithFileUpload: StoryFn<typeof ChatInput> = () => {
  const [files, setFiles] = useState<Klear360FileList>([]);
...
const INITIAL_MESSAGES: ChatMsg[] = [
  {
    ...
    senderType: 'other',
    content:
      'Hello! This is a basic demo with ChatInput and ChatMessage components of Klear360 design system. You can accept the suggestions with tab in input to see different types of responses. Try adding more than 3 files in FileUpload to see error from file upload.',
  },
];
    ...
    <CardBody>
      <Box display="flex" gap="spacing.4" flexDirection="column">
        ...
        <Text variant="body" size="small" color="surface.text.gray.muted">
          Choose where to receive Klear webhook events:
        </Text>
...
const getMockResponse = (userMessage: string): MockResponseResult => {
  ...
  if (/integrate|integration|sdk|api key/i.test(lower)) {
    return {
      type: 'text',
      text:
        'To integrate Klear, start by signing up and grabbing your API keys from the Dashboard. Then install the SDK by running npm install klear in your project. Next, initialize the SDK with your key_id and key_secret. After that, create an Order using the Orders API, and finally open the Klear Checkout on your frontend to accept payments. Would you like a code snippet for a specific language or framework?',
    };
  }
  ...
  return {
    type: 'text',
    text:
      "That's a great question! Klear provides comprehensive APIs, SDKs, and a developer dashboard to help you build and manage payments seamlessly. Is there a specific area you'd like to dive deeper into — like subscriptions, refunds, or settlements?",
  };
};
...
export const ProductUsecaseChatExperience: StoryFn<typeof ChatInput> = () => {
  ...
  const handleSubmit = useCallback(
    ({ value, fileList }: { value: string; fileList: Klear360FileList }) => {
    ...
    >
      ...
      >
        ...
        <Box display="flex" flexDirection="column">
          ...
          <Text variant="body" size="small" color="surface.text.gray.muted">
            {isGenerating ? 'Typing...' : 'Klear AI Assistant'}
          </Text>
        </Box>
      </Box>
...
export const WithFileDismissDuringUpload: StoryFn<typeof ChatInput> = () => {
  const [files, setFiles] = React.useState<Klear360FileList>([
    {
      name: 'report.pdf',
      size: 204800,
      status: 'uploading',
...
export const WithManyFiles: StoryFn<typeof ChatInput> = () => {
  const [files, setFiles] = React.useState<Klear360FileList>([
    ...
    { name: 'screenshot.png', size: 76160, status: 'success', id: 'file-3' } as Klear360FileList[0],
    { name: 'invoice.pdf', size: 512000, status: 'success', id: 'file-4' } as Klear360FileList[0],
    {
