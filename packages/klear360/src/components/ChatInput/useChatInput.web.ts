...
const useChatInput = (
  ...
  const textareaRef = React.useRef<Klear360ElementRef>(null);
  ...
  const [files, setFiles] = useControllableState<Klear360FileList>({
  ...
  const handleFileInputChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      const inputFiles = Array.from(event.target.files ?? []) as Klear360FileList;
  ...
  const handlePaste = React.useCallback(
    (event: React.ClipboardEvent<HTMLInputElement>) => {
      ...
      for (const file of clipboardFiles) {
        if (!(file as Klear360File).id) {
          (file as Klear360File).id = `${Date.now()}${Math.floor(Math.random() * 1000000)}`;
        }
      }

      if (clipboardFiles.length > 0) {
        const newFileList = [...files, ...clipboardFiles] as Klear360FileList;
        setFiles(() => newFileList);
        onFileChange?.({ fileList: newFileList });
      }
    },
