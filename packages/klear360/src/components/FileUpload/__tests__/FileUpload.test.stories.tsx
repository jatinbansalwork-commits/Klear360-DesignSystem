...
import type { Klear360FileList, Klear360File, FileUploadProps } from '~components/FileUpload';
...
export const TestFileUploadError: StoryFn<typeof FileUpload> = (): React.ReactElement => {
  ...
  const uploadFile = async (file: Klear360File, fileList: Klear360FileList): Promise<Response> => {
    setUploadedFiles(
      fileList.map((f) => {
        if (f.id === file.id) {
          f.status = 'uploading';
