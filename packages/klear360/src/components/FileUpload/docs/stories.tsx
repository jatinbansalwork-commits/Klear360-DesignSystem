...
function App() {
    const [selectedFile, setSelectedFile] = useState<Klear360File>();
    ...
    const handleSubmit = (e) => {
      ...
      if (selectedFile) {
        ...
        data.append('upload_preset', 'klear360-file-upload-demo');
...
const MultiFileUploadStory = `
 import {
  ...
  function App() {
    const [selectedFiles, setSelectedFiles] = useState<Klear360FileList>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [responseData, setResponseData] = useState<
      {
        url?: string;
    ...
    const uploadFile = (file: Klear360File): Promise<Response> => {
      const data = new FormData();
    ...
    const handleSubmit = (e) => {
      ...
      if (selectedFiles.length > 0) {
        Promise.all(selectedFiles.map((file: Klear360File) => uploadFile(file)))
          .then((res) => {
            setResponseData(res);
            setIsLoading(false);
          })
...
import {
  ...
  function App() {
    ...
    const [uploadedFiles, setUploadedFiles] = useState<Klear360FileList>([]);
    const [responseData, setResponseData] = useState([]);
    const [isSubmitted, setIsSubmitted] = useState(false);

    const uploadFile = (file: Klear360File, fileList: Klear360FileList): Promise<Response> => {
      setUploadedFiles(
        fileList.map((f: Klear360File) => {
          if (f.id === file.id) {
            f.status = 'uploading';
          }
          return file;
...
const AutoFileUploadWithProgressStory = `
 import {
  ...
  function App() {
    ...
    const uploadFile = (file: Klear360File, fileList: Klear360FileList): void => {
      setUploadedFiles(
        fileList.map((f: Klear360File) => {
          if (f.id === file.id) {
            f.status = 'uploading';
    ...
    const handleFileChange:
      ...
      const unUploadedFiles = fileList.filter((file: Klear360File) => !file.status);
      void Promise.all(
        unUploadedFiles.map((file: Klear360File) => uploadFile(file, fileList))
      );
    };
            ...
            >
              ...
              <Button
                ...
                isDisabled={
                  ...
                  uploadedFiles?.some(
                    (file: Klear360File) => file.status === 'uploading'
                  )
                }
          ...
          {!isSubmitted ? (
            ...
            >
              ...
              <FileUpload
                ...
                accept=".jpg, .jpeg, .png"
                fileList={uploadedFiles}
                onChange={({ fileList }: { fileList: Klear360FileList }) =>
                  handleFileChange({ fileList })
                }
                onDrop={({ fileList }: { fileList: Klear360FileList }) =>
                  handleFileChange({ fileList })
                }
                onPreview={({ file }: { file: Klear360File }) => {
                  setIsOpen(true);
