...
import type { Klear360File, Klear360FileList, FileUploadProps } from '../FileUpload';
...
const createKlear360File = (
  mock: typeof MOCK_FILES[number],
  initialStatus: Klear360File['status'] = 'uploading',
): Klear360File => {
  return {
    name: mock.name,
    size: mock.size,
...
const SingleUploadTemplate: StoryFn<typeof FileUploadComponent> = () => {
  ...
  const handleFilesSelected = useCallback(
    (selected: Array<typeof MOCK_FILES[number]>) => {
      const newFile = createKlear360File(selected[0]);
      setFiles([newFile]);
...
const MultiUploadTemplate: StoryFn<typeof FileUploadComponent> = () => {
  ...
  const handleFilesSelected = useCallback(
    (selected: Array<typeof MOCK_FILES[number]>) => {
      const newFiles = selected.map((s) => createKlear360File(s));
