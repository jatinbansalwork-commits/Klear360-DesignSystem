...
const _FileUpload: React.ForwardRefRenderFunction<Klear360ElementRef, FileUploadProps> = (
  {
    name: _name,
    accept,
    uploadType = 'single',
  ...
  const assignedFileIdsRef = useRef(new WeakMap<Klear360File, string>());

  const [filesWithIds, setFilesWithIds] = useState<Klear360FileList>([]);
  ...
  useEffect(() => {
    setFilesWithIds(
      selectedFiles.map((file) => {
        ...
        const id = `klear360-file-${fileIdCounterRef.current}`;
  ...
  const removeFileFromSelection = (file: Klear360File): void => {
    const newFiles = filesWithIds.filter(({ id }) => id !== file.id);
    setSelectedFiles(() => newFiles);
  };

  const handleFileRemove = (file: Klear360File): void => {
    removeFileFromSelection(file);
    onRemove?.({ file });
  };

  const handleFileReupload = (file: Klear360File): void => {
    removeFileFromSelection(file);
  ...
  const handleFileDismiss = (file: Klear360File): void => {
    removeFileFromSelection(file);
    onDismiss?.({ file });
  };
...
export { FileUpload };
export type { Klear360File, Klear360FileList, FileUploadProps };
