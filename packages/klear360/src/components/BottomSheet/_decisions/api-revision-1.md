...
<BottomSheet isOpen={boolean} snapPoints={[]}>
  <BottomSheetHeader>
    <BottomSheetHeaderLeading title="Payments Links" prefix={Klear360Icon} />
...
Alex proposed to keep the API similar to ActionList
...
```jsx
import { Spinner, useTheme, useBreakpoint } from "@klear/klear360";
...
Me, Alex & Riley was discussing about the potential DX improvements & API simplification we can do if we don’t care about the bundle size issue.

**Alex’s proposed API**

Alex suggested an API similar to Dropdown:
...
**Riley & I also discussed the “Ideal” API:
...
- Discussed with @Klear about the "clear" button in the header, design will need to revisit this section and change it accordingly (Action Item)
...
- We decided it's best to keep these ActionListHeader & BottomSheetHeader separate because:
    ...
    - It will also make the API a bit awkward since not all the time we will render the ActionList inside BottomSheet nor could it be same all the time for mweb or dweb, for example:
...
- For composition with BottomSheet with Dropdown, we decided to keep the **BottomSheet an independent component** to ensure these usecases are also solved without adding any bundle size of Dropdown or ActionList.
...
- **Design:** @Klear to relook at the BottomSheet Header/Footer components and see if the "clear" button in the header is required or not.
