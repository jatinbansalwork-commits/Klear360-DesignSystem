...
- [Figma - BottomSheet](https://www.figma.com/file/jubmQL9Z8V7881ayUD95ps/Klear360---Payment-Light?node-id=26477%3A578228&t=1RMqjs99P33Udj0C-0)
...
```jsx
import { BottomSheet } from '@klear/klear360';
...
- From klear360 side, we will have greater control over the pattern
...
- These usecases will be harder to solve for without the BottomSheet being an independent component.
...
```jsx
import { useTheme, useBreakpoint, BottomSheet, Dropdown } from '@klear/klear360';
...
**Q.** Why can't we lazy load from klear360 side?

There are two major reasons:

1. It's generally not feasible to lazy load components from library side, We can't possibly know where & how a klear360 component can be used or imported in the user's codebase. Even if we know that it will also require changes in how we bundle klear360, since right now klear360 distributes the bundle in single index.js chunks, introducing lazy loading will create more fragmented chunks.
...
const BottomSheet = React.lazy( => import('@klear/klear360/components/bottomsheet'));
const DropdownOverlay = React.lazy( => import('@klear/klear360/components/dropdown-overlay'));
