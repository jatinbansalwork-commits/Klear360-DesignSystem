...
describe('<BottomSheet />', () => {
  ...
  test('BottomSheetHeader trailing should not allow any random component', () => {
    ...
    expect(() => renderWithTheme(<Example />)).toThrow(
      '[Klear360: Header]: Only one of `Button, IconButton, Badge, Link, Text, Amount` component is accepted as trailing',
    );
    mockConsoleError.mockRestore();
  });
  ...
  test('BottomSheetHeader trailing should warn about prop overrides', () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        '[Klear360: Header]: Do not pass "size" to "Badge" while inside Header trailing, because we override it.',
      ),
    );
