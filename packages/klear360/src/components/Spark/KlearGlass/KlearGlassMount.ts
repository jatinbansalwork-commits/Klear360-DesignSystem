 ...
 * KlearGlassMount
 *
 * Core vanilla class for the KlearGlass WebGL shader effect.
 ...
 */
import { klearGlassVertexShader, klearGlassFragmentShader } from './klearGlassShader';
import type { KlearGlassConfig, KlearGlassAssets } from './types';
...
const defaultStyle = `@layer klear-glass {
...
export class KlearGlassMount {
  ...
  // Configuration
  private config: Required<KlearGlassConfig>;
  private assets: KlearGlassAssets;
  ...
  constructor(
    ...
    assets: KlearGlassAssets,
    config: Partial<KlearGlassConfig> = {},
  ...
  private setUniformValues(config: Partial<KlearGlassConfig>): void {
    const gl = this.gl;
    gl.useProgram(this.program);
   ...
   */
  public setUniforms(newConfig: Partial<KlearGlassConfig>): void {
