...
import { KlearGlassMount } from './KlearGlassMount';
...
const KlearGlass = forwardRef<HTMLDivElement, KlearGlassProps>(function KlearGlass(
  props,
  forwardedRef,
) {
  const {
  ...
  // Follow Klear360's color scheme: in dark mode every preset gets its dark
  ...
  const mountRef = useRef<KlearGlassMount | null>(null);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        mountRef.current = new KlearGlassMount(
          divRef.current,
          {
            videoSrc,
            imageSrc,
