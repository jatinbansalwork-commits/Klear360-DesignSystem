...
const getDefaultAssets = (assetsPath: string): Required<KlearGlassAssets> => ({
 ...
 */
function extractConfig(props: KlearGlassProps): Partial<KlearGlassConfig> {
  const {
    width: _width,
    height: _height,
...
function getPresetDefinition(
  preset: KlearGlassPreset | undefined,
  assetsPath: string,
  isDark = false,
): KlearGlassPresetDefinition {
  const presets = getPresets(assetsPath);
  const definition: KlearGlassPresetDefinition =
...
function getPresetAssets(
  ...
  assetsPath: string,
  isDark = false,
): Partial<KlearGlassAssets> {
  const def = getPresetDefinition(preset, assetsPath, isDark) as Record<string, unknown>;
  return Object.fromEntries(
    Object.entries(def).filter(([k]) => ASSET_KEYS.has(k)),
  ) as Partial<KlearGlassAssets>;
}
...
/**
 * Preload all assets for a given KlearSense preset.
 ...
 * @param assetsPath - Optional CDN path for assets (defaults to Klear360 CDN)
 * @param colorScheme - Color scheme to preload for. Pass `'dark'` to preload the
 * dark gradient maps. Since this runs outside React it cannot read Klear360's color
 ...
 * await preloadKlearSenseAssets('circleSlideUp');
 *
 * // Now mount the component - assets are already cached
 * <KlearSense preset="circleSlideUp" />
 *
 * // In dark mode, preload the dark asset set
 * const { colorScheme } = useTheme();
 * await preloadKlearSenseAssets('circleSlideUp', undefined, colorScheme);
 * ```
 */
async function preloadKlearSenseAssets(
  preset: KlearGlassPreset = 'default',
