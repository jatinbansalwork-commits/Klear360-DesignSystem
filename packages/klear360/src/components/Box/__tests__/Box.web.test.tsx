...
describe('<Box />', () => {
  ...
  it('should throw error for unsupport as prop', () => {
    ...
    try {
      ...
      expect(err).toMatchInlineSnapshot(
        `[Error: [Klear360: Box]: Invalid \`as\` prop value - button. Only div, section, footer, header, main, aside, nav, span, label are valid values]`,
      );
    }
    console.error = tempConsoleError;
  });
  ...
  it('should not throw error and render properly with undefined backgroundColor', () => {
    ...
    expect(container).toMatchInlineSnapshot(`
      .c0.c0.c0.c0.c0 {
        background-color: hsla(217,62%,30%,1);
      }
