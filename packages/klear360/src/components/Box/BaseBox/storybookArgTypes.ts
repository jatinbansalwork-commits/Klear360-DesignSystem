...
import type { BaseBoxProps, BoxProps, StyledPropsKlear360 } from './types';
...
} = {}): StorybookArgTypes<StyledPropsKlear360> => {
...
> => {
  return {
    ...
    borderRadius: {
      ...commonProperties,
      description: '**`borderRadius` token of klear360**.',
    },
    lineHeight: {
      ...commonProperties,
...
const getKlear360CommonEventArgTypes = (): StorybookArgTypes<Record<string, string>> => {
  return {
    onMouseEnter: {
      table: {
