...
TestOverflow.play = async ({ canvasElement }) => {
  ...
  await expect(queryByRole('link', { name: 'Express Checkout' })).toBeNull();
  await expect(queryByRole('menuitem', { name: 'Express Checkout' })).toBeVisible();
  ...
  await expect(queryByRole('menuitem', { name: 'Express Checkout' })).toBeNull();
