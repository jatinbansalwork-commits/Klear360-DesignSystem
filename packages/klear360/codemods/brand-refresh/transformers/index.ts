...
const transformer: Transform = (file, api, options) => {
  ...
  // Don't transform if the file doesn't import `@klear/klear360/components` because it's not using Klear360 components
  // Allow the migration test file to be transformed
  if (!newSource.includes('@klear/klear360/components') && file.path !== undefined) {
    return newSource;
  }
  ...
  // Update the themeTokens prop in Klear360Provider
  try {
    root
      .find(j.JSXElement, {
        openingElement: {
          name: {
            name: 'Klear360Provider',
          },
        },
      })
      .find(j.JSXAttribute, {
      ...
      .replaceWith(({ node }) => {
        node.value.expression.name = 'klear360Theme';
    ...
    console.error(
      red(
        `⛔️ ${file.path}: Oops! Ran into an issue while updating the themeTokens prop in Klear360Provider.`,
      ),
      `\n${red(error.stack)}\n`,
    );
  }
  ...
  // Update ImportSpecifier from "paymentTheme"/"bankingTheme" to "klear360Theme"
  try {
    root
      .find(j.ImportDeclaration, {
        source: {
          value: '@klear/klear360/tokens',
        },
      })
      ...
      .replaceWith((path) => {
        path.value.imported.name = 'klear360Theme';
    ...
    console.error(
      red(
        `⛔️ ${file.path}: Oops! Ran into an issue while updating the ImportSpecifier from "paymentTheme"/"bankingTheme" to "klear360Theme".`,
      ),
      `\n${red(error.stack)}\n`,
    );
  }
