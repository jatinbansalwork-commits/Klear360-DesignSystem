 ...
 * no longer had them, `klear360-core` left stale because only `klear360` was in the target list, and a
...
const GITHUB_BOT_EMAIL = 'tools+cibot@klear.com';
const GITHUB_BOT_USERNAME = 'klearcibot';

const KLEAR360_ROOT = path.resolve(__dirname, '..');
const REPO_ROOT = path.resolve(KLEAR360_ROOT, '../..');
...
/**
 * Writes one token tree into every file that mirrors it. `klear360` and `klear360-core` hold identical
 * copies of the theme files; writing only one of them is what left `klear360-core` failing to
...
const buildPullRequestBody = ({ added, removed, consumers, report }) => {
  ...
  if (preservedTokens.size) {
    sections.push(
      ...
      } were kept as-is because Figma does not define them, per \`preservedTokenPaths\` in \`packages/klear360/scripts/tokenTargets.json\`.`,
      '',
...
const uploadColorTokens = async () => {
  ...
  for (const themeName of Object.keys(themeColorTokens)) {
    if (!targets.themes[themeName]) {
      blockers.push(
        `The payload contains a theme called \`${themeName}\` with no target files configured. Add it to \`packages/klear360/scripts/tokenTargets.json\`, otherwise it is never written anywhere.`,
      );
    }
  }
  ...
  execa.sync('yarn', ['prettier', '--write', ...touchedFiles], { cwd: KLEAR360_ROOT });
