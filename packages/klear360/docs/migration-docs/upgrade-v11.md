...
```mermaid
flowchart LR
  subgraph design["Step 1: Design changes (always)"]
    A[Pull in the changes on Figma] --> B[Update Figma designs as per<br/>the upgrade guide, one page at a time]
  end
  subgraph dev["Step 2: Dev changes"]
    C[Follow the upgrade guide and use<br/>the codemod to automigrate a page] --> D{Any issues not covered<br/>in the upgrade handbook?}
  end
  B --> C
  D -- No --> E[Visual QA]
  D -- Yes --> F[Raise it with the design systems team]
  E --> A
```

## Migration with Codemod

**Step 1:** Install this version of Klear360 as `yarn add @klear/klear360-rebranded@npm:@klear/klear360@v11.0.0`.

**Step 2:** Install Inter, Roboto, and Roboto Mono by following [this file](https://klear360.klear.com/?path=/docs/guides-installation--docs#-installing-fonts).

**Step 3:** The codemod will update the components to the new version of Klear360. Execute the codemod on the file/directory that needs to be migrated for the page via the following command:

> Need help? Check out [jscodeshift docs](https://github.com/facebook/jscodeshift) for CLI usage tips.

```sh
npx jscodeshift ./PATH_TO_YOUR_DIR --extensions=tsx,ts,jsx,js -t ./node_modules/@klear/klear360-rebranded/codemods/brand-refresh/transformers/index.ts --ignore-pattern="**/node_modules/**"
```
...
- The codemod doesn't handle the migration of conditionally rendered props. Take a moment to manually inspect and update such cases. The codemod will also log a warning for such cases with the line number & path to the file. For example: `Expression found in the "size" attribute, please update manually: src/pages/ResumeWithKlear/sections/WhyResumeSection.tsx:20`
...
By default, `klear360.klear.com` will show documentation for the latest version of Klear360. To view the documentation for an older version, you can use the version selector in the top-left corner of the page.


## Available Rebranded Components

To check out the list of available components, visit [Klear360 Component Status](https://klear360.klear.com/?path=/docs/guides-component-status--docs).
...
- **`paymentTheme` & `bankingTheme` have been removed. Use `klear360Theme` instead.**

  ```diff
    import { Klear360Provider } from '@klear/klear360/components';
  - import { paymentTheme, bankingTheme } from "@klear/klear360/tokens";
  + import { klear360Theme } from "@klear/klear360/tokens";

    const AppWrapper = () => {
      return (
  -     <Klear360Provider themeTokens={paymentTheme|bankingTheme} colorScheme="light">
...
- **Amount component is now internationalized via [@klear/i18n](https://www.npmjs.com/package/@klear/i18n).**
...
2. If you want to enable users to change the locale of your page, add the `@klear/i18n-react` package and wrap your app inside the `I18nProvider`. Utilize the `setI18nState` utility to modify the locale. For more details, please refer to the [documentation](https://www.npmjs.com/package/@klear/i18n-react).

3. Additionally, if you prefer to maintain a fixed locale for your page and amount component, enclose your app within `<I18nProvider initData={{locale: 'locale-you-want'}}>..`. For more details, please refer to the [documentation](https://www.npmjs.com/package/@klear/i18n-react).

Example with `@klear/i18n-react`

```jsx
import React, { useEffect } from 'react';
import { I18nProvider, useI18nContext } from '@klear/i18n-react';
import { Klear360Provider, Amount } from '@klear/klear360/components';
...
- **The `"default"` value for the `color` prop has been removed in favor of the new `"primary"` value.**
  ...
  - <Link color="default" href="https://klear.com/"> Hello </Link>
  + <Link color="primary" href="https://klear.com/"> Hello </Link>
  ```
