# Migration to Motion Presets (Klear360 v12)
...
Klear360 v12 introduces `framer-motion` as peer dependency and requires you to set it up in your projects.

## Steps to Migrate

- **Step 1:** Upgrade to latest `@klear/klear360` package in your project
...
```sh
npx jscodeshift ./PATH_TO_YOUR_DIR --extensions=tsx,ts,jsx,js -t ./node_modules/@klear/klear360/codemods/migrate-motion-tokens/transformers/index.ts --ignore-pattern="**/node_modules/**"
```
...
> [!IMPORTANT]
>
> `framer-motion` library is now known as `motion/react`. We still use `framer-motion` imports in klear360 to support older versions.
>
...
We realised that several projects in klear are already using `framer-motion` and are on older versions.
To give some time to consumers to upgrade to framer-motion v11+, we'll be supporting framer-motion v4+ from klear360. Although we will be dropping this support in next major version of klear360 so we recommend planning out framer-motion upgrade in coming quarter.
...
- **For projects not on React 18 yet**, do plan out the upgrade soon to make sure future klear360 upgrades don't become blocker
...
- #### Setup reduced bundle version of `framer-motion`
  ...
  function App() {
    return (
      ...
      // Klear360 presets always use `m` while animating
  ...
  ##### Go ahead and enjoy the Klear360 Motion Presets

  ```ts
  import { Fade, Badge } from '@klear/klear360/components';
...
Got stuck somewhere or have some doubts? reach out to `@klear360-developers` on `#design-system` channel
