...
import { useTheme, Klear360Provider, Code, Text, Alert } from '../../src/components';
...
<Unstyled>
  <Alert
    description="The corresponding screens mentioned in the table are under assumption that you're using min-width query over token values. Internally in Klear360 we use min-width queries for responsive props in Box and in useBreakpoint hook. If you're importing breakpoint tokens and using some other media query, the corresponding screen may change"
    isFullWidth
    marginY="spacing.6"
    isDismissible={false}
  />
